...
def generate_random_network(n_edges=100, n_vert=20, seed=124, sort=True):
    rng = np.random.default_rng(seed=seed)
    tail = rng.integers(low=0, high=n_vert, size=n_edges)
    head = rng.integers(low=0, high=n_vert, size=n_edges)
