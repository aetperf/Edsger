...
No decrease-key operation - simplified for lazy Dijkstra and other algorithms.
...
- extract_min
    Extract element with min key from the heap,
    and return its element index.
- copy_keys_to_numpy
    Copy the keys into a numpy array.
- _exchange_nodes
    Exchange two nodes in the heap (simplified - no node_idx updates).
- _min_heapify
...
from edsger.commons cimport (
    DTYPE_INF, LABELED, UNLABELED, DTYPE_t
)
...
cdef void _initialize_element(
    ...
    * size_t element_idx : index of the element in the element array
    ...
    # NO node_idx - not needed without decrease-key
...
cdef void insert(
    ...
    * DTYPE_t key : key value of the element
    ...
    assumptions
    ===========
    * the new key is smaller than DTYPE_INF
    """
    cdef:
        size_t i = pqueue.size
        size_t j, gp
        DTYPE_t key_j

    pqueue.size += 1
    pqueue.Elements[element_idx].state = LABELED
    pqueue.Elements[element_idx].key = key
    pqueue.A[i] = element_idx

    # Inline bubble-up (no separate function needed)
    while i > 0:
        j = (i - 1) // 4  # parent in 4-ary tree
...
cdef size_t extract_min(PriorityQueue* pqueue) noexcept nogil:
    ...
    * pqueue.size > 0
    ...
    # Note: state is NOT set to SCANNED here - that's done in Dijkstra
    # after checking if this is a stale duplicate
...
cdef void _exchange_nodes(
    ...
    Simplified version - no node_idx updates needed without decrease-key.
    ...
    * size_t node_j: second node index
    """
    cdef size_t tmp = pqueue.A[node_i]
    pqueue.A[node_i] = pqueue.A[node_j]
    pqueue.A[node_j] = tmp
    # NO node_idx updates - not needed without decrease-key
...
cdef void heapsort(DTYPE_t[::1] values_in, DTYPE_t[::1] values_out) noexcept nogil:
    """
    Heap sort by inserting all the values into the priority queue,
    and extracting them.
    """
