...
def create_siouxfalls_network():
    """
    Create a directed graph representing the Sioux Falls network.

    This function reads a string in CSV format containing the edges of the Sioux Falls network,
    where each row represents an edge from the tail node to the head node with a corresponding
    travel time.
    ...
    Examples
    --------
    >>> graph_edges = create_siouxfalls_network()
